...
if [ ! -s ${SAMPLE_DIR}/ds_region_with_BC_bam.txt ]; then # if no ds calls, write empty VCF
  ...
  bcftools view -H -v indels -R ${SAMPLE_DIR}/ds_region_with_BC_bam.txt --regions-overlap 0 ${SAMPLE_DIR}/${SAMPLE}.vcf.gz | \
  if [ $(wc -l - | cut -d ' ' -f1) -ne $(wc -l ${SAMPLE_DIR}/ds_region_with_BC_bam.txt | cut -d ' ' -f1) ]; then 
    echo ${SAMPLE} "ERROR: mismatching double-stranded variant numbers between indel_calls and intermediate vcf"; continue;
  else
    bcftools view -H -v indels -R ${SAMPLE_DIR}/ds_region_with_BC_bam.txt --regions-overlap 0 ${SAMPLE_DIR}/${SAMPLE}.vcf.gz | \
    awk ' BEGIN{FS="\t";OFS="\t"} {\
...
if [ ! -s ${SAMPLE_DIR}/ss_region_with_BC_bam.txt ]; then # if no ss calls, write empty VCF
  ...
  bcftools view -H -v indels -R ${SAMPLE_DIR}/ss_region_with_BC_bam.txt --regions-overlap 0 ${SAMPLE_DIR}/${SAMPLE}.vcf.gz | \
  if [ $(wc -l - | cut -d ' ' -f1) -ne $(wc -l ${SAMPLE_DIR}/ss_region_with_BC_bam.txt | cut -d ' ' -f1) ]; then 
    echo ${SAMPLE} "ERROR: mismatching single-stranded variant numbers between indel_calls and intermediate vcf"; continue;
  else
    bcftools view -H -v indels -R ${SAMPLE_DIR}/ss_region_with_BC_bam.txt --regions-overlap 0 ${SAMPLE_DIR}/${SAMPLE}.vcf.gz | \
    awk ' BEGIN{FS="\t";OFS="\t"} {\
      if(NR==1) print "##fileformat=VCFv4.0\n#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO"
